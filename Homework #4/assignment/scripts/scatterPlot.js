...
function ScatterPlot(svg, data, updateFlowDiagram) {
    ...
    function computeCorrelation(data) {
        ...
        var denom = d3.sum(data, (d) => { return d.X * d.X }) * d3.sum(data, (d) => { return d
                .Y * d.Y });
    ...
    var xAxisContainer = svg.append("g")
        .attr("class", "xAxis")
        .attr("transform",
            `translate(${margins.left}, ${svgHeight-margins.top})`);
    ...
    this.draw = function(newData) {
        ...
        update(selection.enter().append("circle").transition(), "enter",
            "green");
